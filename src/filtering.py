...
def apply_signal_cleaning_pipeline(df: pd.DataFrame,
    ...
    # Ticket 011: accumulate per-frame OR mask across all position columns.
    # True at frame t iff any position column had a Hampel replacement at frame t.
    # Saved in pipeline_metadata for NB04 to persist as {RUN_ID}__s04_hampel_or_mask.npy.
    _t011_hampel_or = np.zeros(n_frames_total, dtype=bool) if n_frames_total > 0 else np.array([], dtype=bool)
    ...
    for col in pos_cols_valid:
        ...
        # Ticket 011: accumulate into session-level OR mask
        if len(hampel_outliers) == len(_t011_hampel_or):
            _t011_hampel_or |= hampel_outliers
    ...
    # Ticket 011: include per-frame OR mask for NB04 to persist and NB06 to consume
    pipeline_metadata['hampel_or_frame_mask'] = _t011_hampel_or
