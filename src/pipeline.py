...
def run_pipeline(csv_path, schema, seg_map, run_id="run1", cfg=CONFIG, output_root="analysis"):
    ...
    try:
        ...
        # Ticket 004: parse session label columns from csv_path stem (UD-006 Option A)
        import re as _re_t004_pipeline
        _label_re_t004p = _re_t004_pipeline.compile(r'^(\d+)_(T\d+)_(P\d+)_(R\d+)')
        _csv_stem = os.path.splitext(os.path.basename(str(csv_path)))[0]
        _lm_t004p = _label_re_t004p.match(_csv_stem)
        _t004_subject_id = _lm_t004p.group(1) if _lm_t004p else None
        _t004_timepoint  = _lm_t004p.group(2) if _lm_t004p else None
        _t004_piece      = _lm_t004p.group(3) if _lm_t004p else None
        _t004_rep        = _lm_t004p.group(4) if _lm_t004p else None
        ...
        df_full, df_viz = build_master_tables(
            ...
            subject_id=_t004_subject_id,
            timepoint=_t004_timepoint,
            piece=_t004_piece,
            rep=_t004_rep,
        )
        ...
        err_str = str(e)
        print(f"Pipeline failed: {err_str}")
        # Ticket 002: detect S01 hard FAIL gate and write fail report
        if "S01_GATE_FAIL" in err_str:
            import re as _re
            def _extract(key):
                m = _re.search(rf"{key}=([^:]+)", err_str)
                return m.group(1) if m else None
            _fail_report = {
                "run_id": run_id,
                "gate_01_status": "FAIL",
                "fail_reason": _extract("reason"),
                "n_frames": int(_extract("n_frames") or 0),
                "duration_sec": float(_extract("duration_sec") or 0),
                "threshold_frames": 3600,
                "threshold_duration_sec": 30.0,
                "error_message": err_str,
            }
            _step01_dir = os.path.join(cfg.get("derivatives_dir", "derivatives"), "step_01_parse")
            os.makedirs(_step01_dir, exist_ok=True)
            write_json(os.path.join(_step01_dir, f"{run_id}__s01_fail_report.json"), _fail_report)
            return {"status": "FAIL", "gate_01_status": "FAIL", "error": err_str}
        return {"status": "FAIL", "error": err_str}
    finally:
        gc.collect()