...
def _export_3stage(
    ...
    # Ticket 007b: per-joint Hampel modification fraction (aggregated across x/y/z axes).
    # per_joint_results is keyed by column name like 'Hips__px'. We aggregate to joint level.
    _t007b_hampel_threshold = 0.05  # 5% per-joint modification rate is the alert threshold
    _t007b_per_joint_hampel_sum: Dict[str, int] = {}
    _t007b_per_joint_n_frames: Dict[str, int] = {}
    for col, meta in per_joint.items():
        joint = col.split('__')[0] if '__' in col else col
        n_outliers = int(meta.get('stage2_hampel_outliers', 0))
        _t007b_per_joint_hampel_sum[joint] = _t007b_per_joint_hampel_sum.get(joint, 0) + n_outliers
        # Track total frames per joint (sum across axes)
        _t007b_per_joint_n_frames[joint] = _t007b_per_joint_n_frames.get(joint, 0) + n_frames
    _t007b_hampel_frac_per_joint: Dict[str, float] = {}
    for joint, n_out in _t007b_per_joint_hampel_sum.items():
        total = _t007b_per_joint_n_frames.get(joint, 0)
        _t007b_hampel_frac_per_joint[joint] = round(float(n_out) / total, 6) if total > 0 else 0.0
    _t007b_hampel_max = max(_t007b_hampel_frac_per_joint.values()) if _t007b_hampel_frac_per_joint else 0.0
    _t007b_joints_above = sorted([
        j for j, f in _t007b_hampel_frac_per_joint.items()
        if f > _t007b_hampel_threshold
    ])
    ...
    summary = {
        ...
        "filter_params": {
            ...
            # Ticket 007b: per-joint Hampel modification summary
            "hampel_modification_fraction_per_joint": _t007b_hampel_frac_per_joint,
            "hampel_max_fraction_any_joint": round(float(_t007b_hampel_max), 6),
            "hampel_joints_above_threshold": _t007b_joints_above,
            "hampel_threshold_fraction": _t007b_hampel_threshold,
