...
JOINT_GROUPS: Dict[str, List[str]] = {
    # Ticket 010 (Phase 7 F-012 / Phase 10 LD-12): Hips excluded from ATF_axial.
    # Rationale: Hips is the root joint; lin_vel_rel_mag is 0 by definition for the
    # root (root-relative coordinates → root's own root-relative position is always 0,
    # so its derivative is always 0). Including Hips in the axial group's median ATF
    # systematically biased ATF_axial downward. Empirical Dev Set impact: +0.0%–1.28%.
    "axial": ["Spine", "Spine1", "Neck", "Head"],
    "peripheral": [
...
def compute_quality_gates(
    ...
    Gate thresholds from config:
        artifact_critical_threshold  0.20  hard exclude (Ticket 008: tightened from 0.30)
        artifact_warning_threshold   0.20  soft flag (now equal to critical; tier collapsed)
    ...
    # Ticket 008: tightened from 0.30 → 0.20 per Phase 7 F7-4. Reference threshold
    # for hard_exclude is now stricter so reference candidates with >20% artifact
    # frames are rejected. Note: art_warn (0.20) is now equal to art_crit, which
    # collapses the soft_warning tier (intentional — any borderline session is now
    # hard_exclude). The legacy artifact_critical_threshold = 0.30 default is gone.
    art_crit = float(config.get("artifact_critical_threshold", 0.20))
    ...
    for run_id, df in sessions.items():
        ...
        # Ticket 006: dead session gate — hard_exclude immediately without artifact computation.
        # Ticket 002 prevents dead sessions from producing parquets, but this guard is
        # defensive for any session that reaches here with < 3600 frames.
        if n_frames < 3600:
            rows.append({
                "run_id": run_id,
                "n_frames": n_frames,
                "duration_s": duration_s,
                "global_artifact_frac": 1.0,
                "clean_fraction_pca": 0.0,
                "clean_duration_s": 0.0,
                "hard_exclude": True,
                "soft_warning": False,
                "pca_low_confidence": True,
                "short_session": True,
                "joints_excluded": list(ALL_19_JOINTS),
                "endpoints_flagged": list(TM_ENDPOINTS),
                "joint_artifact_rates": {j: 1.0 for j in ALL_19_JOINTS},
            })
            logger.warning(
                "compute_quality_gates: dead session '%s' (n_frames=%d < 3600) → hard_exclude=True",
                run_id, n_frames,
            )
            continue
        ...
        # All-joint clean mask (PCA requirement) — computed FIRST per Ticket 008
        # because global_artifact_frac now derives from it.
        ...
        # Ticket 008: OR-union artifact fraction = fraction of frames where ANY joint
        # has an artifact (Phase 7 F7-3). Equals 1 - clean_fraction_pca by construction.
        # Replaces the previous `max(joint_art_rates.values())` which understated the
        # true PCA-readiness burden when artifacts in different joints fall at different
        # frames. The OR-union is the correct measure for whole-body PCA admissibility.
        global_artifact_frac = (1.0 - clean_fraction_pca) if n_frames > 0 else 0.0
...
def build_pca_engine(
    ...
    # Step 4–5: Transform all sessions (Ticket 006: skip hard-excluded sessions)
    for run_id, df in sessions.items():
        if excluded_run_ids and run_id in excluded_run_ids:
            logger.warning(
                "build_pca_engine: skipping hard_excluded session '%s'", run_id
            )
            continue
