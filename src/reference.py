...
def compute_q_ref_and_ref_qc(time_s, q_local, ref_info, joints_export_idx, joints_viz_idx, cfg):
    ...
    if ref_info.get("t_pose_failed", False):
        ...
        qc = {
            # Ticket 005: None serializes to JSON null; float("nan") produces invalid JSON NaN.
            "identity_error_ref_med": None,
    ...
    # Ticket 005: serialize NaN as None (JSON null) not as float NaN (invalid JSON).
    _identity_med_out = None if (not np.isfinite(identity_med)) else float(identity_med)
    _ref_quality_out = None if (not np.isfinite(ref_quality_score)) else float(ref_quality_score)

    qc = {
        "identity_error_ref_med": _identity_med_out,
        "ref_quality_score": _ref_quality_out,
