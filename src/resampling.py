...
def resample_time_grid(time_s, fs_target):
    """
    Build a uniform time grid spanning [t0, t1] inclusively at fs_target Hz.

    Ticket 003: uses n = round((t1-t0)*fs)+1 to preserve both endpoints
    (the previous half-open `np.arange(t0, t1, dt)` dropped the final frame).

    Defensive boundary clamp: integer arithmetic on (t1-t0)*fs followed by
    division can push the last grid point microscopically past t1 due to
    floating-point precision. Downstream interpolators that require strictly
    inclusive bounds (e.g. scipy.spatial.transform.Slerp) reject such points,
    so we clamp the final value to t1 exactly.
    """
    t0 = float(time_s[0])
    t1 = float(time_s[-1])
    n = int(round((t1 - t0) * fs_target)) + 1
    grid = t0 + np.arange(n) / fs_target
    if len(grid) > 0:
