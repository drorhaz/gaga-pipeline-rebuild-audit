...
class PipelineRunner:
    ...
    def process_single_csv(self, csv_path: Path) -> Dict:
        ...
        # Ticket 001: Write frozen config snapshot BEFORE update_config() mutates config_v1.yaml.
        # Captures the exact parameters that will govern this run, before per-session fields
        # (current_csv, subject anthropometrics) are overwritten.
        try:
            with open(self.config_file, 'r') as _f:
                _snapshot_cfg = yaml.safe_load(_f) or {}
            _snapshot_cfg['snapshot_timestamp'] = datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%SZ')
            _snapshot_dir = self.project_root / "derivatives" / "step_00_config"
            _snapshot_dir.mkdir(parents=True, exist_ok=True)
            _snapshot_path = _snapshot_dir / f"{csv_path.stem}__config_snapshot.yaml"
            with open(_snapshot_path, 'w') as _f:
                yaml.dump(_snapshot_cfg, _f, default_flow_style=False, allow_unicode=True)
            self.logger.info(f"Config snapshot written: {_snapshot_path.name}")
        except Exception as _e:
            self.logger.warning(f"Config snapshot failed (non-fatal): {_e}")
        ...
        for notebook_num in self.pipeline_sequence:
            ...
            if result['status'] == 'failed':
                all_success = False
                # Ticket 002: write S01 FAIL gate report if notebook 01 raised S01_GATE_FAIL
                if notebook_num == '01' and 'S01_GATE_FAIL' in str(result.get('error', '')):
                    self._write_s01_fail_report(csv_path, str(result.get('error', '')))
    ...
    def _write_s01_fail_report(self, csv_path: Path, error_str: str):
        """Ticket 002: Write S01 FAIL gate report when notebook 01 fails with a gate-fail exception."""
        import re as _re
        run_id = csv_path.stem
        deriv_01 = self.project_root / "derivatives" / "step_01_parse"
        deriv_01.mkdir(parents=True, exist_ok=True)

        # Extract values from the structured S01_GATE_FAIL line only.
        # The papermill error contains the source code f-string (with {T}) in the traceback,
        # so we must search within the actual ValueError line: "ValueError: S01_GATE_FAIL:..."
        gate_line_match = _re.search(r"ValueError:\s*(S01_GATE_FAIL:[^\n]+)", error_str)
        gate_line = gate_line_match.group(1) if gate_line_match else error_str

        def _extract(key):
            m = _re.search(rf"{key}=([^:\n]+)", gate_line)
            return m.group(1).strip() if m else None

        n_frames_raw = _extract("n_frames")
        duration_raw = _extract("duration_sec")
        report = {
            "run_id": run_id,
            "gate_01_status": "FAIL",
            "fail_reason": _extract("reason"),
            "n_frames": int(n_frames_raw) if n_frames_raw and n_frames_raw.isdigit() else None,
            "duration_sec": float(duration_raw) if duration_raw else None,
            "threshold_frames": 3600,
            "threshold_duration_sec": 30.0,
            "error_message": gate_line,
            "timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ"),
        }
        report_path = deriv_01 / f"{run_id}__s01_fail_report.json"
        with open(report_path, 'w') as _f:
            json.dump(report, _f, indent=2)
        self.logger.info(f"S01 FAIL gate report written: {report_path.name}")
